  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Business info */}
        <div className="card">
          <h2 className="font-semibold text-gray-900 dark:text-white mb-4">Información del Negocio</h2>
          <form onSubmit={saveSettings} className="space-y-4">
            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nombre del Negocio</label>
            ...
            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">NIT</label>
              <input className="input-field" value={settings.business_nit || ""} onChange={e => setSettings({...settings, business_nit: e.target.value})} /></div>
            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Dirección</label>
            ...
            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Teléfono</label>
        ...
        <div className="space-y-6">
          <div className="card">
            <h2 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><DollarSign className="w-5 h-5" /> Facturación</h2>
            <form onSubmit={saveSettings} className="space-y-4">
              <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Prefijo de Factura</label>
              ...
              <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Próximo Número</label>
              ...
              <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tasa de IVA</label>
              ...
              <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Moneda (símbolo)</label>
          ...
          {/* Cash session */}
          <div className="card">
            <h2 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><Banknote className="w-5 h-5" /> Caja Actual</h2>
            {cashSession ? (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><span className="text-gray-500 dark:text-gray-400">Monto Inicial:</span> <span className="font-semibold">{formatCurrency(cashSession.openingAmount)}</span></div>
                  <div><span className="text-gray-500 dark:text-gray-400">Ventas:</span> <span className="font-semibold text-emerald-600">{formatCurrency(cashSession.salesTotal)}</span></div>
                </div>
                <button onClick={() => setShowCashClose(true)} className="btn-danger w-full">Cerrar Caja</button>
              </div>
            ) : (
              <p className="text-sm text-gray-500 dark:text-gray-400 py-4 text-center">No hay caja abierta. Abra una desde el Punto de Venta.</p>
            )}
          </div>
        </div>
      </div>
      ...
      <Modal open={showCashClose} onClose={() => setShowCashClose(false)} title="Cerrar Caja" size="sm">
        {cashSession && (
          <form onSubmit={closeCash} className="space-y-4">
            ...
            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Monto Contado en Caja</label>
