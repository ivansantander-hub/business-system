  ...
  if (!cashSession) {
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] gap-6">
        ...
        <ShoppingCart className="w-16 h-16 text-gray-300 dark:text-gray-500" />
        <h2 className="text-xl font-semibold text-gray-600 dark:text-gray-300">Caja cerrada</h2>
        <p className="text-gray-400 dark:text-gray-400">Debe abrir la caja para comenzar a vender</p>
    ...
    <div className="flex gap-4 h-[calc(100vh-120px)]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
            ...
            {tabs.map(t => (
              <button key={t.id} onClick={() => setActiveTab(t.id)}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                  activeTab === t.id ? "bg-indigo-600 text-white" : "bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600"
                }`}>
        ...
        {activeTab === "memberships" && (
          <div className="flex-1 overflow-y-auto">
            ...
            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Planes disponibles</h3>
        ...
        {activeTab === "daypasses" && (
          <div className="flex-1">
            <div className="max-w-md space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Miembro (opcional)</label>
                <input className="input-field" placeholder="Buscar miembro..." value={memberSearch}
                  onChange={e => setMemberSearch(e.target.value)} />
                {gymMembers.length > 0 && memberSearch.length >= 2 && (
                  <div className="mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg max-h-32 overflow-y-auto">
                    {gymMembers.map(m => (
                      <button key={m.id} onClick={() => { setSelectedMemberId(m.id); setMemberSearch(m.customer.name); setGymMembers([]); }}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nombre del invitado (si no es miembro)</label>
      ...
      <div className="w-96 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {cart.length === 0 ? (
            <p className="text-sm text-gray-400 dark:text-gray-400 text-center py-12">Agrega productos al carrito</p>
          ) : cart.map(item => (
            <div key={item.product.id} className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3">
              <div className="flex items-start justify-between">
                <p className="text-sm font-medium text-gray-900 dark:text-white flex-1">{item.product.name}</p>
              ...
              <div className="flex items-center justify-between mt-2">
                {item.product.id > 0 ? (
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => updateQuantity(item.product.id, 1)} className="w-7 h-7 rounded-full bg-indigo-100 dark:bg-indigo-900/20 flex items-center justify-center hover:bg-indigo-200 dark:hover:bg-indigo-800/30 text-indigo-600 dark:text-indigo-400">
                      <Plus className="w-3 h-3" />
                    </button>
                  </div>
                ) : (
                  <span className="text-xs text-indigo-600 dark:text-indigo-400 font-medium">{item.product.category?.name}</span>
                )}
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 p-4 space-y-2">
          <div className="flex justify-between text-sm text-gray-500 dark:text-gray-400"><span>Subtotal</span><span>{formatCurrency(subtotal)}</span></div>
          <div className="flex justify-between text-sm text-gray-500 dark:text-gray-400"><span>IVA (19%)</span><span>{formatCurrency(tax)}</span></div>
